...
export async function fetchRecipesWithOptions(query: string, limit: number): Promise<Recipe[]> {
    try {
        const baseUrl = 'https://api.edamam.com/api/recipes/v2/';
        const typeVal = 'public';
        const apiUrl = `https://api.edamam.com/api/recipes/v2?type=${typeVal}&q=${query}&app_id=${process.env.NEXT_PUBLIC_APP_ID}&app_key=${process.env.NEXT_PUBLIC_APP_KEY}&limit=${limit}`;
        const response = await fetch(apiUrl);
        if (!response.ok) {
            throw new Error(`Failed to fetch data: ${response.status} ${response.statusText}`);
        }
